...
def create_directory_hierarchy(
    ...
    for d in dicts:
        ...
        for development in d.pop("development"):
            ...
            change_filename = Path(dev_item_path, f"{file_name}.md")
            with open(change_filename, "w", encoding="utf-8") as file:
                if change_sets := development["change_sets"]:
                    for change_set in change_sets:
                        changed_file_name = change_set.get("artifact_file_name")
                        artifacts_file = Path(attachments_path, changed_file_name)
                        shutil.move(artifacts_file, dev_item_path)

                        file.write(
                            f"* 'Original File Name': {change_set['File Name']}\n"
                        )
                        file.write(f"* 'Path': {change_set['Path']}\n")
                        file.write(
                            f"* 'Downloaded Artifact File Name': {changed_file_name}\n"
                        )

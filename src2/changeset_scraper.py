...
def download_changeset(driver, changeset_downloads):
    browse_files = find_element_by_xpath(driver, "//a[@role='link']")
    driver.execute_script("arguments[0].click();", browse_files)

    download_btn = find_element_by_xpath(driver, "//button[@data-command-key='downloadAsZip']")
...
def get_all_changeset_urls(driver):
    ...
    while scroll_init <= content_height + (scroll_increment * 2):
        ...
        # Get urls
        changeset_body = find_element_by_xpath(driver, "//div[contains(@class, 'changes-history-list-container')]")
        changesets = find_elements_by_xpath(changeset_body, "//div[@role='row']//a")
...
def scrape_changeset(driver, changeset_downloads, id_folders, changesets_dir):
    ...
    for changeset_url in changeset_urls:
        ...
        _id = changeset_url.split("/")[-1]
        print("Change set ID: ", _id)

        if _id in id_folders:
            print("Skipping existing ID: ", _id)
            continue
